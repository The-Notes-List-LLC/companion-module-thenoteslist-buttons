...
vi.mock('@companion-module/base', () => {
  class InstanceBase {
    ...
    checkFeedbacks() {}
    log() {}
    saveConfig(c: unknown) { h.saved.push(c) }
...
const PAIRED = { baseUrl: 'https://notes.test', token: 't', startPairing: false } as Record<string, unknown>
...
describe('polling', () => {
  it('polls counts every 5 s and /me every 60 s once paired', async () => {
    await inst.configUpdated(PAIRED)
  ...
  it('a re-save while /me is in flight does not double polling', async () => {
    ...
    const first = inst.configUpdated(PAIRED)
    await flush()
    const second = inst.configUpdated(PAIRED)
    await flush()
...
describe('pairing', () => {
  ...
  it('starts the paired session itself after a successful pair', async () => {
    ...
    expect(h.saved.at(-1)).toMatchObject({ token: 'new', startPairing: false })
    expect(h.status.at(-1)?.[0]).toBe('ok')
    calls = []
