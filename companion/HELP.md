...
Enter the desk's IP in **Eos desk IP** (tick **Use TCP SLIP** if the desk uses port 3037 / OSC 1.1; the Eos default is port 3032). Enable OSC RX/TX on the desk under Setup → System → Show Control → OSC. This module then follows the live cue of cue list 1 (change **Cue list** if needed) and fetches only a small window of cues around it, one request at a time, so even a 1700-cue show costs the desk a dozen tiny messages per Go. It never subscribes to the desk's parameter stream and never sends a command to it.
